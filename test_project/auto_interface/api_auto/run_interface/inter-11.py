...
class run_word():
    def run_reqerst_http(self, run_request_id):
        ...
        if control_num:
            ...
            run_work_lis.append(run_request_id)
        units = unittest.TestSuite()
        units.addTest(Testinterfexec("itest_"))
        ht = unittest.TextTestRunner().run(units)
        print("----------")
        print(ht)
    ...
    def read_lis(self):
        id_test = 1
        for run_id_data in interfase_run_listid:
            run_work_lis.append(["%s" % id_test, run_id_data])
        print("run_work_lis 执行列表 %s" % run_work_lis)
        return run_work_lis
        ...
        if autoin_assert_lists.count() != 0:
            ...
            for autoin_assert_list in autoin_assert_lists:
                ...
                if autoin_assert_list["right_contrast_int"] == '':
                    ...
                    getattr(self, autoin_assert_list["assert_name"])(eval("t." + autoin_assert_list["left_contrast"]),
                                                                     dat_json1)
